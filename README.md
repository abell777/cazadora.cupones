...
Al probar contra Booking.com real, la extensión detectó correctamente el
carrito pero no encontró el cupón que sí tenías guardado para esa tienda.
Causa: el checkout de Booking vive en `secure.booking.com`, pero el cupón
estaba guardado bajo `booking.com` — `normalizeDomain()` solo quitaba el
prefijo `www.`, así que un subdominio distinto no coincidía nunca con el
dominio base guardado.

- **`background/background.js`**: `normalizeDomain()` ahora calcula el
  dominio "base" real (p. ej. `secure.booking.com` → `booking.com`,
  `checkout.tienda.co.uk` → `tienda.co.uk`), en vez de solo quitar `www.`.
  Afecta a la vez a la búsqueda de cupones y al sellado de afiliado (que
  tenía el mismo problema, aunque no se notase visualmente).
- **`popup/popup.js`**: tenía su propia copia de la función con el mismo
  fallo; ahora usa la misma lógica, así que "Tienda actual" en el popup
  también muestra el dominio base en vez del subdominio del checkout.

No es infalible al 100% (los sufijos tipo `.co.uk` se detectan contra una
lista corta hecha a mano, no la Public Suffix List completa), pero cubre
los casos reales que nos hemos encontrado y los mercados objetivo del
proyecto.
...
## Cambios en este paso (v0.13.0 → v0.13.1): bug crítico — el aviso no salía solo en checkouts reales

Al probar contra el checkout real de Cecotec, la extensión no ofreció el
aviso flotante por sí sola; hubo que abrirla a mano desde el icono. Causa:
`content.js` solo comprobaba la página **una vez**, 800ms después del evento
`load`. Si el campo de cupón se renderiza más tarde (checkouts que cargan el
carrito por JavaScript después de esa comprobación) o si la tienda navega
entre páginas sin recargar del todo (SPA), la extensión se quedaba "ciega"
para siempre en esa página.

- **`content/content.js`**: ahora vigila la página de forma continua en vez
  de mirar una sola vez: reacciona al instante a cambios reales en el DOM
  (`MutationObserver`) y además hace un sondeo de refuerzo cada 1,5s durante
  30s por si el cambio no dispara el observer. También detecta navegación
  de tipo SPA (enganchando `pushState`/`replaceState`/`popstate`) para
  volver a comprobar sin que haga falta recargar la página. Evita mostrar
  el aviso más de una vez por URL.

Los cupones `CECOBIENVENIDA` / `CECOTEC10` que salieron como "unknown" al
probarlos en Cecotec real es lo esperado: son códigos de ejemplo, no reales
— confirma que el siguiente paso lógico es sustituir la semilla por cupones
reales.

## Cambios en el paso anterior (v0.12.0 → v0.13.0): scraper automático por IA de Telegram/foros
